...
#define NUMBER_OF_FRAMES 2000 
  int timeOutLogTime=20; // [ms]
  
  int loggingArray[NUMBER_OF_FRAMES][4]; // [time point][h;min;s;ms]
  int frameNumber=1;
...
//    Convert();                         // Convert image got.
  ...
  std::ofstream allTimesFile ("C:\\AppLogs\\SC2_demo_all_times.txt", std::ofstream::out);
  std::ofstream TimeOutTimesFile ("C:\\AppLogs\\SC2_demo_timeout_times.txt", std::ofstream::out);
...
DWORD CSC2Class::SC2Thread2()
{
  int ierr = 0;
  int ierrorcnt = 0;
  WORD wimage = 0;

#define NUMBER_OF_FRAMES 50000 
  ...
  ResetEvent(ghEvents[1]);
	...
	ierr = PCO_AddBufferEx(hCam, wimage, wimage, (SHORT)0, m_wXResAct, m_wYResAct, 16);
	ierr = PCO_AddBufferEx(hCam, wimage, wimage, (SHORT)1, m_wXResAct, m_wYResAct, 16);
  ...
  while (m_bThreadRun & frameNumber<=NUMBER_OF_FRAMES)
  {
	...
	ierr = WaitForMultipleObjects(2, ghEvents, FALSE, 2000);
    switch (ierr)
    {
      case WAIT_OBJECT_0:
        ierr = PCO_GetBufferStatus(hCam, 0, &dwStatusDll, &dwStatusDrv);
		...
		ierr = PCO_AddBufferEx(hCam, wimage, wimage, 0, m_wXResAct, m_wYResAct, 16);
		ResetEvent(ghEvents[0]);
      ...
      case WAIT_OBJECT_0+1:
        ierr = PCO_GetBufferStatus(hCam, 1, &dwStatusDll, &dwStatusDrv);
        if (ierr != PCO_NOERROR)
        {
          ::PostMessage(m_hwndMain, WM_APP+101, 0, ierr);
          break;
		...
		ierr = PCO_AddBufferEx(hCam, wimage, wimage, 1, m_wXResAct, m_wYResAct, 16);
		ResetEvent(ghEvents[1]);
...
DWORD CSC2Class::SC2Thread3()
{
	int ierr = 0;
	int ierrorcnt = 0;
	WORD wimage = 0;

	#define NUMBER_OF_FRAMES 10000
	int timeOutLogTime=19; // [ms]
...
DWORD CSC2Class::SC2Thread4()
{
	int ierr = 0;
	int ierrorcnt = 0;
	WORD wimage = 0;

	#define NUMBER_OF_FRAMES 2499
	#define NUMBER_OF_TIME_POINTS 20

	int timeOutLogTime=19; // [ms]
  
	int loggingArray[NUMBER_OF_FRAMES][5][NUMBER_OF_TIME_POINTS]; // [time point][h;min;s;ms]
	int frameNumber=1;
	...
	for (int time_index=0;time_index<NUMBER_OF_TIME_POINTS;time_index++)
	{
		...
		while (m_bThreadRun & frameNumber<=NUMBER_OF_FRAMES)
		{
		  ...
		  //        break;
				...
				GetSystemTime(&systime);
				loggingArray[frameNumber-1][0][time_index] = (int)systime.wHour;
				loggingArray[frameNumber-1][1][time_index] = (int)systime.wMinute;
				loggingArray[frameNumber-1][2][time_index] = (int)systime.wSecond;
				loggingArray[frameNumber-1][3][time_index] = (int)systime.wMilliseconds;
				loggingArray[frameNumber-1][4][time_index] = buffer_number;
...
/*
	...
	for (int time_index=0;time_index<NUMBER_OF_TIME_POINTS;time_index++)
	{
		...
		for(int f_ind=0;f_ind<NUMBER_OF_FRAMES;f_ind++){
			allTimesFile<<f_ind+1<<"\t";
			for (int i=0;i<5;i++){
				allTimesFile<<loggingArray[f_ind][i][time_index]<<"\t";
			}	  
			allTimesFile<<"\n";

			if (f_ind>0 & ((loggingArray[f_ind][0][time_index]*60*60*1000+loggingArray[f_ind][1][time_index]*60*1000+loggingArray[f_ind][2][time_index]*1000+loggingArray[f_ind][3][time_index])-(loggingArray[f_ind-1][0][time_index]*60*60*1000+loggingArray[f_ind-1][1][time_index]*60*1000+loggingArray[f_ind-1][2][time_index]*1000+loggingArray[f_ind-1][3][time_index]))>timeOutLogTime) {
				TimeOutTimesFile<<f_ind<<"\t";
				for (int i=0;i<5;i++){
					TimeOutTimesFile<<loggingArray[f_ind-1][i][time_index]<<"\t";
				}	  
				TimeOutTimesFile<<"\n";

				TimeOutTimesFile<<f_ind+1<<"\t";
				for (int i=0;i<5;i++){
					TimeOutTimesFile<<loggingArray[f_ind][i][time_index]<<"\t";
				}	  
				TimeOutTimesFile<<"\n";
			}
		}
